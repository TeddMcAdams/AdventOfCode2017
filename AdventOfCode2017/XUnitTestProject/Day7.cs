...
namespace XUnitTestProject
{
    ...
    {
        #region Puzzle Input
        private readonly string _input1 = @"pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)";
        ...
        private readonly string _input2 = @"dihjv (2158) -> gausx, ncdmp, hozgrub
...
ihhmv (72)";
        ...
        {
            //Part1
            Assert.Equal("tknk", FindBottomProgram(_input1));
        ...
        {
            var startingInfo = ParseInputString(input);

            var allNames = startingInfo.Select(m => m.name);
            var allAboveSomeone = startingInfo.SelectMany(m => m.namesOfThoseAbove);

            return allNames.Single(m => !allAboveSomeone.Contains(m));
        } 

        private List<(string name, int weight, List<string> namesOfThoseAbove)> ParseInputString(string input)
        {
            string name, weight, thoseAbove = null;
            var namePattern = new Regex(@"^\w+");
            var weightPattern = new Regex(@"\((\d+)\)");
            var thoseAbovePattern = new Regex(@"-> (.+)");

            var startingInfo = new List<(string name, int weight, List<string>)>();
            ...
            {
                name = namePattern.Match(row).Value;
                weight = weightPattern.Match(row).Groups[1].Value;
                thoseAbove = thoseAbovePattern.Match(row).Groups[1].Value;

                startingInfo.Add((name, int.Parse(weight), thoseAbove.Split(", ").ToList()));
            }
